...
export default function Toolkit(){
 ...
 async function run(){
  ...
  try{
   if(mode==="images-pdf"){const out=await PDFDocument.create();for(const f of files){const bytes=await f.arrayBuffer();let img; if(f.type==="image/png")img=await out.embedPng(bytes); else img=await out.embedJpg(bytes);const p=out.addPage([img.width,img.height]);p.drawImage(img,{x:0,y:0,width:img.width,height:img.height})}download(new Blob([await out.save().then(bytes => bytes.buffer)],{type:"application/pdf"}),"images.pdf")}
   if(mode==="merge"){const out=await PDFDocument.create();for(const f of files){const src=await PDFDocument.load(await f.arrayBuffer());const pages=await out.copyPages(src,src.getPageIndices());pages.forEach(p=>out.addPage(p))}download(new Blob([await out.save()],{type:"application/pdf"}),"merged.pdf")}
   if(mode==="split"||mode==="rotate"){const src=await PDFDocument.load(await files[0].arrayBuffer());const out=await PDFDocument.create();let indexes=src.getPageIndices();if(mode==="split"&&range){indexes=range.split(",").flatMap(x=>{const [a,b]=x.trim().split("-").map(Number);return b?Array.from({length:b-a+1},(_,i)=>a-1+i):[a-1]}).filter(i=>i>=0&&i<src.getPageCount())}const pages=await out.copyPages(src,indexes);pages.forEach(p=>{if(mode==="rotate")p.setRotation(degrees((p.getRotation().angle+rotation)%360));out.addPage(p)});download(new Blob([await out.save()],{type:"application/pdf"}),mode==="split"?"split.pdf":"rotated.pdf")}
